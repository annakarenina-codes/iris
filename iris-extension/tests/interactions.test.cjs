...
test('Recent checks load collapsed, expand in place, and leave when history clears', async (t) => {
  ...
  assert.equal(await page.locator('.recent-checks__count').textContent(), '2');
  ...
  assert.equal(await page.locator('.recent-check__summary').count(), 2, 'Rows appear when the section opens');
