...
- **The claim split runs early.** Partitioning a claim into components needs only the claim and the post, so it runs as soon as the claim's search is done, while the post's other claims are still searching (`prepare_component_review`), and only when the search returned an article the review could read. Passages are embedded by the review, for the articles it reads: embedding every readable article ahead of the review, before the gates and the reading budget, doubled the embedding of posts with many claims in the 34-case check.
...
Checked against the full backend tests, a request-by-request comparison of the old and new review (identical when a claim has no restored quotation), and the 34 development cases. That check found three problems of quotation claims searching on their own, fixed the same day (see the change record in [SOURCES.md](SOURCES.md)): straight quotes made Brave search a quotation as an exact phrase, whole sentences keeping a Filipino quotation ran past Brave's query limit, and articles shared between claims were ranked on the Filipino words alone.
